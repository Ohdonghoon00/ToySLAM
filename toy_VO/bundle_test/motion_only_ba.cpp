...
std::cout << "after track_feature num  : " << current_image.pts.size() << endl;            
            ...
            current_image.cam_pose = homogenous_campose_to_vec6d(cam_pose_initialize_);
...
std::cout << map_storage.keyframe[keyframe_num].pts.size() <<endl;
                ...
                //     double* camera = (double*)(&map_storage.keyframe[0].cam_pose_);
                ...
                // std::cout << " camera cam pose before intialize ba " << endl <<vec6d_to_homogenous_campose(map_storage.keyframe[0].cam_pose_) << endl;

                // ceres::Solve(options, &initilize_ba, &summary);

                // std::cout << " camera cam pose after intialize ba " << endl <<vec6d_to_homogenous_campose(map_storage.keyframe[0].cam_pose_) << endl;
...
std::cout << " map point size for motion only BA : " << map_point_for_motion_BA.size() << endl;
            ...
            track_inlier_ratio = 100 * current_image.pts.size() / map_storage.keyframe[keyframe_num].pts.size();
std::cout << "  track inlier ratio : " << track_inlier_ratio << endl;
            ...
            if (new_keyframe_selection)
            {
                previous_image.pts.clear();
                cv::goodFeaturesToTrack(previous_image.frame, previous_image.pts, 2000, 0.01, 10);
std::cout << " New Feature  !!!!  num is : " << previous_image.pts.size() << endl;
                track_opticalflow_and_remove_err(previous_image.frame, current_image.frame, previous_image.pts, current_image.pts);
std::cout << "after track_feature num  : " << current_image.pts.size() << endl;
...
std::cout << " map point size this keyframe after remove outlier  : " << map_point.size() << endl;
            ...
            if (new_keyframe_selection)
            {
                ...
                cv::solvePnPRansac(map_point, current_image.pts, K, cv::noArray(), rot, tran, false, 100, 3.0F, 0.99, inliers_ );
                inlier_storage.push_back(inliers_);
std::cout << keyframe_num <<" keyframe inlier storage rate : " << 100 * inliers_.rows / map_point.size() << endl;                
...
std::cout << "@@@@@@@@@ New keyframe was made @@@@@@@@@" << endl << " Keyframe num is  : " << keyframe_num << endl;   
                ...
                //             double* camera = (double*)(&map_storage.keyframe[j].cam_pose_);
                ...
                // for(int j = 0; j < keyframe_num + 1; j++) std::cout << " keyframe num : " << j << endl << vec6d_to_homogenous_campose(map_storage.keyframe[j].cam_pose_) << endl;
...
/////////////////////            VISUALIZE         ////////////////////////////////////////////////////////////////
        ...
        {   
            ...
            show_map_point_parms = map_storage.world_xyz.size();
std::cout << " map_storage size is : " << map_storage.world_xyz.size();
        }
