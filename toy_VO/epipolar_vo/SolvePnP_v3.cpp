...
{       
    ...
    while(true)
    {
         ...
         if (times > 1)
        {
            ...
            cv::solvePnP(map_point, corresponding_points, K, dist_coeff, World_R, World_t);
        ...
        cv::recoverPose(E, previous_image.image_pts, current_image.image_pts, triangulate_R, triangulate_t, f, c, inlier_mask);
        ...
        cv::String info = cv::format("Inliers: ,  XYZ: [%.3f, %.3f, %.3f]", _cam_pose_.at<double>(0, 0),_cam_pose_.at<double>(0, 1),_cam_pose_.at<double>(0, 2));
