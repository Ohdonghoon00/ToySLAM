...
{       
    ...
    while(true)
    {
        ...
        if (times > 1)
        {
            ...
            cv::solvePnPRansac(map_point, corresponding_points, K, cv::noArray(), World_R, World_t);
        ...
        std::cout << previous_image.image_pts.size() << endl;
        ...
        inlier_num = cv::recoverPose(E, previous_image.image_pts, current_image.image_pts, triangulate_R, triangulate_t, f, c, inlier_mask);
        cv::hconcat(triangulate_R, triangulate_t, triangulate_Rt);
        cv::vconcat(triangulate_Rt, Iden.row(3), triangulate_Rt);
        if ( 100 * inlier_num/ previous_image.image_pts.size() > 35) cam_pose_ = cam_pose_ * triangulate_Rt.inv();
        ...
        else
        {
            ...
            int solution = cv::decomposeHomographyMat(H, K, triangulate_R_, triangulate_t_, normal);
            triangulate_R = triangulate_R_[0];
            triangulate_t = triangulate_t_[0];
            cv::hconcat(triangulate_R, triangulate_t, triangulate_Rt);
            cv::vconcat(triangulate_Rt, Iden.row(3), triangulate_Rt);
            cam_pose_ = cam_pose_ * triangulate_Rt.inv();
        }
            

        cv::Mat X, P1;
        cv::sfm::projectionFromKRt(K, triangulate_R, triangulate_t, P1);
        ...
        cv::String info = cv::format("Inliers: %d (%d%%) XYZ: [%.3f, %.3f, %.3f]",inlier_num, 100 * inlier_num/ previous_image.image_pts.size(), _cam_pose_.at<double>(0, 0),_cam_pose_.at<double>(0, 1),_cam_pose_.at<double>(0, 2));
        ...
        { 
            std::cout << " Frame_Id :       " << abs[i].Frame_id << "   " << endl;
            std::cout << " Frame_point_id : " << abs[i].frame_pts_id.size() << "   " << endl;
        }
