...
{       
    ...
    while(true)
    {
        ...
        for(int i = 0; i < previous_image.image_pts.size(); i++) previous_image.frame_pts_id.push_back(i);
        ...
        cv::String info = cv::format("Inliers: %d (%d%%),  XYZ: [%.3f, %.3f, %.3f]", inlier_num, 100 * inlier_num/ previous_image.image_pts.size(), cam_pose_.at<double>(0, 3), cam_pose_.at<double>(1, 3), cam_pose_.at<double>(2, 3));
