...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////          
    ...
    int active_keyframe_num = 1;
    int fix_keyframe_num = 15;
...
std::cout << " Rotation difference : " << rotation_difference << endl;           
            ...
            if (track_inlier_ratio < 60 or inliers.rows < 200 or rotation_difference > 0.055 ) new_keyframe_selection = true;
...
std::cout << "translation difference : " << translation_difference << endl;            
            ...
            if (new_keyframe_selection)
            {
                ...
                //     for(int j = optimize_intial_keyframe_num; j < keyframe_num + 1; j++) std::cout << " keyframe num : " << j << endl << vec6d_to_homogenous_campose(map_storage.keyframe[j].cam_pose) << endl;
...
// std::cout << " map_point size after remove solvepnp outlier : " <<  map_point.size() << "  " << current_track_point_for_triangulate.size() << endl;
...
// std::cout << " map_point size after remove solvepnp outlier : " <<  clone_map_point.size() << "  " << clone_current_track_point_for_triangulate.size() << endl;
                
                    int indexCorrect = 0;
                for(int i = 0; i < clone_current_track_point_for_triangulate.size(); i++)
                {
                    // std::cout << previous_track_point_for_triangulate_ID[i] << "    ";
                    ...
                    {
                //   std::cout << map_point[i] <<  "   " << map_storage.world_xyz[previous_track_point_for_triangulate_ID[i]] << endl;
                    // std::cout << map_point[i] <<  "   " << map_storage.world_xyz[previous_track_point_for_triangulate_ID[i]] << endl;
...
std::cout << "@@@@@@@@@ New keyframe was made @@@@@@@@@" << endl << " Keyframe num is  : " << keyframe_num << " Frame num is : " << times << endl;   
                ...
                // {
                //     std::cout << "keyframe num  :  " << i << "  pts size :  " << map_storage.keyframe[i].pts.size() << "    " << "pts id size  :   " << map_storage.keyframe[i].pts_id.size() << " cam pose  :  " << map_storage.keyframe[i].cam_pose << endl;
                // }       
...
std::cout << " fixed keyframe num  : " << j << endl;                         
                ...
                std::cout << " new feature " << endl;
                current_track_point_for_triangulate.clear();
                cv::goodFeaturesToTrack(current_image.frame, current_track_point_for_triangulate, 1500, 0.01, 8);
std::cout << " new feature num  : " << current_track_point_for_triangulate.size() << endl; 
                track_entire_num = current_track_point_for_triangulate.size();
...
/////////////////////            VISUALIZE         ////////////////////////////////////////////////////////////////
        ...
        { 
                ...
                cv::Mat rb_t = homogenous_campose_for_keyframe_visualize(campose_vec6d_to_mat_for_visualize, 15.0); // size
                show_trajectory_keyframe(rb_t, 0.0, 0.0, 0.0, 1.0);
                // if( keyframe_num - fix_intial_keyfram_num == 20) fix_intial_keyfram_num++;
                // optimize_intial_keyframe_num++;
                // if(keyframe_num - optimize_intial_keyframe_num < 0 ) optimize_intial_keyframe_num = 0;
        ...
        if (times == initialize_frame_num)
        {
            for( int i = 0 ; i < map_storage.keyframe[0].pts_id.size(); i++)
            {
                ...
                show_trajectory(X_map, Y_map, Z_map, 0.0, 0.0, 1.0, 0.01);
