...
std::cout << "( before bundle )current cam pose  : " << endl <<  cam_pose << endl;
            cv::Mat pose_difference = cv::abs(cam_pose) - cv::abs(previous_image.cam_pose_);
...
std::cout << "translation difference : " << translation_difference << endl;
            ...
            // Bundle Adjustment
            cv::Vec6d campose_for_bundle = homogenous_campose_to_vec6d(cam_pose);
            ...
            for ( int i = 0; i < current_image.pts.size(); i++)
            {
                    ...
                    double* camera = (double*)(&campose_for_bundle);
            ...
            cam_pose = vec6d_to_homogenous_campose(campose_for_bundle); 
std::cout << "( after bundle )current cam pose  : " << endl <<  cam_pose << endl;
