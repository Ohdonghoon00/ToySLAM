...
{       
    ...
    cv::Mat _cam_pose_initialize = cv::Mat::eye(4, 4, CV_64F);
...
//////////////        Caculate R, t using ESSENTIAL MATRIX or HOMOGRAPHY      //////////////////////////////////////////// 
        if ( times < initialize_frame_num + 1)
        {
            ...
            P0 = P1.clone();
            _cam_pose_initialize = cam_pose_initialize * Rt.inv();
std::cout << " cam pose ( before bundle ) " << endl << _cam_pose_initialize << endl;
            cv::Mat Rt_ = _cam_pose_initialize.inv();
            Rt_.resize(3);
            P1 = K * Rt_;
            ...
            std::vector<cv::Vec6d> cameras(1);
            cv::Mat rvec;
            cv::Rodrigues(R_, rvec);
            cameras[0] = cv::Vec6d(rvec.at<double>(0, 0), rvec.at<double>(1, 0), rvec.at<double>(2, 0), t_.at<double>(0, 0), t_.at<double>(1, 0), t_.at<double>(2, 0));
            ...
            for ( int i = 0; i < current_image.pts.size(); i++)
            {
                    ...
                    double* camera = (double*)(&cameras[0]);
...
// std::cout << summary.FullReport() << endl;
            ...
            cv::Vec3d _rvec(cameras[0][0], cameras[0][1], cameras[0][2]); 
            cv::Mat _t = (cv::Mat_<double>(3, 1) << cameras[0][3], cameras[0][4], cameras[0][5]);
            cv::Mat _R;
            cv::Rodrigues(_rvec, _R);
            cv::Mat _Rt = R_t_to_homogenous(_R, _t);
// std::cout << " camera pose (after bundle)  : " << endl << cam_pose_initialize << endl; 

            
            // Update cam_pose using R, t
            cam_pose_initialize = cam_pose_initialize * _Rt.inv();
...
std::cout << " current cam pose : ( after bundle ) " << endl << cam_pose_initialize << endl;
            ...
            cv::projectPoints(project_mat, _R, _t, K, cv::noArray(), projectionpoints);                
...
std::cout << " Inlier ratio : " << 100 * inliers.rows / map_point.size() << " %" << endl;
            ...
            track_inlier_ratio = 100 * current_image.pts.size() / keyframe_storage[keyframe_num].keyframe.pts.size();
...
std::cout << "@@@@@@@@@ New keyframe was made @@@@@@@@@" << endl << " Keyframe num is  : " << keyframe_num << endl; 
        ...
        // Storage map point to keyframe
        if (times == initialize_frame_num)
        {
            Map storage_map(keyframe_storage[0], map_point);
            map_storage.push_back(storage_map);
        }
        if (new_keyframe_selection)
        {
            Map storage_map(keyframe_storage[keyframe_num], map_point);
            map_storage.push_back(storage_map);
...
/////////////////////            VISUALIZE         ////////////////////////////////////////////////////////////////
        ...
        if (times == initialize_frame_num)
        {
            ...
            {
                GLfloat X_map(map_storage[keyframe_num].world_xyz[i].x), Y_map(map_storage[keyframe_num].world_xyz[i].y), Z_map(map_storage[keyframe_num].world_xyz[i].z);
