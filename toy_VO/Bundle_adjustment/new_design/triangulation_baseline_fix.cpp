...
//////////////        Caculate R, t using ESSENTIAL MATRIX or HOMOGRAPHY      //////////////////////////////////////////// 
        if ( times < initialize_frame_num + 1)
        {
            ...
            cv::goodFeaturesToTrack(previous_image.frame, previous_image.pts, 2000, 0.01, 15);
...
std::cout << " frame storage size ::: " <<  frame_storage.size() << endl;
            if (new_keyframe_selection)
            {
                ...
                previous_image.pts.clear();
                cv::goodFeaturesToTrack(map_storage.keyframe_[keyframe_num].frame, previous_image.pts, 2000, 0.01, 15);
std::cout << " new feature num  : " << previous_image.pts.size() << endl;
                track_opticalflow_and_remove_err(map_storage.keyframe_[keyframe_num].frame, current_image.frame, previous_image.pts, current_image.pts);
std::cout << "after track_feature num  : " << current_image.pts.size() << endl;
...
std::cout << vec6d_to_homogenous_campose(map_storage.keyframe_[keyframe_num].cam_pose_) << endl;
std::cout << vec6d_to_homogenous_campose(current_image.cam_pose_) << endl;

                P0 = K * cam_storage_to_projection_matrix(map_storage.keyframe_[keyframe_num].cam_pose_);
...
std::cout << " map point size this keyframe before remove outlier  : " << map_point.size() << endl;
                cam_pose_ = vec6d_to_homogenous_campose(current_image.cam_pose_);
                remove_map_point_and_2dpoint_outlier(map_point, current_image.pts, cam_pose_);
...
std::cout << "map_point size  : "  << map_point.size() << endl;
                ...
                for(int j = 0; j < map_point.size(); j++)
                {
                    ...
                    for(int i = 0; i < clone_map_storage_world_xyz.size(); i++)
                    {
                        ...
                        if (map_point_difference_x < 2.0 and map_point_difference_y < 2.0 and map_point_difference_z < 2.0 and map_point_difference < 5)
                        {
...
/////////////////////            VISUALIZE         ////////////////////////////////////////////////////////////////
        ...
        // GLdouble _x_cam_pose(current_image.cam_pose_.at<double>(0, 3)), _y_cam_pose(current_image.cam_pose_.at<double>(1, 3)), _z_cam_pose(current_image.cam_pose_.at<double>(2, 3));
        ...
        if (times <= initialize_frame_num)
        {
            ...
            GLdouble x_cam_pose(campose_vec6d_to_mat_for_visualize.at<double>(0, 3)), y_cam_pose(campose_vec6d_to_mat_for_visualize.at<double>(1, 3)), z_cam_pose(campose_vec6d_to_mat_for_visualize.at<double>(2, 3));            
            show_trajectory(x_cam_pose, y_cam_pose, z_cam_pose, 0, 0, 0, 3.0);
            show_trajectory_mini(x_cam_pose, y_cam_pose, z_cam_pose, 0, 0, 0, 2.0);           
        }
        if (new_keyframe_selection)
        { 
            for(int j = show_trajectory_parms; j < times + 1; j++) 
            {
                cv::Mat campose_vec6d_to_mat_for_visualize = vec6d_to_homogenous_campose(frame_storage[j].cam_pose_);
                GLdouble x_cam_pose(campose_vec6d_to_mat_for_visualize.at<double>(0, 3)), y_cam_pose(campose_vec6d_to_mat_for_visualize.at<double>(1, 3)), z_cam_pose(campose_vec6d_to_mat_for_visualize.at<double>(2, 3));
                show_trajectory(x_cam_pose, y_cam_pose, z_cam_pose, 0, 0, 0, 3.0);
                show_trajectory_mini(x_cam_pose, y_cam_pose, z_cam_pose, 0, 0, 0, 2.0);
                show_trajectory_parms = times + 1;
            }
        }
        ...
        if (times == initialize_frame_num)
        {
            std::cout << " map_point size in show image " << map_point.size() << endl;

            for( int i = 0 ; i < map_point.size(); i++)
            {
                GLdouble X_map(map_point[i].x), Y_map(map_point[i].y), Z_map(map_point[i].z);
